...
- **☝️ 手指向上指**（握拳姿势）→ 向下滑动 → 切换到**上一个视频**
- **👇 手指向下指**（握拳姿势）→ 向上滑动 → 切换到**下一个视频**

> **注意**：
> 1. 抖音的滑动逻辑与手势方向相反（向上滑显示下一个视频），因此手指向下指会触发向上滑动来切换到下一个视频
> 2. 必须保持握拳姿势（食指伸直，其他三指弯曲）才能触发手势，防止误触
...
| **食指伸直检测** | ✅ 已实现 | `GestureRecognitionService.kt` |
| **握拳检测（其他手指弯曲）** | ✅ 已实现 | `GestureRecognitionService.kt` |
| **灵敏度调节（界面可调）** | ✅ 已实现 | `HomeFragment.kt` |
...
使用食指的 **掌指关节(MCP)**、**近端指间关节(PIP)**、**远端指间关节(DIP)**、**指尖(TIP)** 四个关键点判断指向方向：

```
关键点索引（MediaPipe 定义）：
- 5: 食指掌指关节 (Index Finger MCP)
- 6: 食指近端指间关节 (Index Finger PIP)
- 7: 食指远端指间关节 (Index Finger DIP)
- 8: 食指指尖 (Index Finger TIP)

方向判断逻辑（Android 屏幕坐标系：y 向下增加）：
1. 检查食指是否伸直（防误触）
   - 计算 MCP→PIP、PIP→DIP、DIP→TIP 三段的方向角度
   - 三段角度差均小于 40° → 食指伸直 ✓
   - 任意段角度差 ≥ 40° → 食指弯曲，忽略手势 ✗
2. 计算指尖到近端指间关节 (PIP) 的向量角度
3. 角度 240°~300° → 向上指 ☝️（指尖在关节上方，对应 atan2 约 270°）
4. 角度 60°~120° → 向下指 👇（指尖在关节下方，对应 atan2 约 90°）
5. 其他角度 → 无明确方向
```
...
**方法**：
- 伸直状态：三段关节方向一致，角度差 < 伸直阈值（默认 40°）
- 弯曲状态：任一段与其他段方向差异 ≥ 伸直阈值

**可调参数**：
- 伸直严格度：范围 20°-60°，越小越难触发（需要手指更直）

**示例**：
- 伸直的食指：MCP→PIP→DIP→TIP 几乎成一直线，各段角度差约 0-20°
- 握拳状态：PIP→DIP 或 DIP→TIP 段会明显弯曲，角度差 > 40°

### 3.3 握拳检测原理

**目的**：确保用户在食指伸直时其他手指呈握拳姿势，提高识别准确性，防止无意触发

**方法**：
- 检测中指（9-12）、无名指（13-16）、小指（17-20）是否都弯曲
- 弯曲判断：各关节段角度差 > 弯曲阈值（默认 45°）

**可调参数**：
- 握拳严格度：范围 20°-80°，越小要求握拳越紧
- 握拳要求开关：可禁用握拳检测（仅检测食指伸直）

**手势要求**：
1. 食指伸直（各段角度差 < 伸直阈值）
2. 其他手指弯曲（各段角度差 > 弯曲阈值）
3. 满足上述条件后才能触发向上/向下手势
...
// 伸直检测阈值（度数）- 三段关节方向允许的最大角度差
// 范围 20°-60°，越小越严格（更难触发），越大越宽松（容易误触）
const val STRAIGHT_THRESHOLD = 40f

// 弯曲检测阈值（度数）- 手指弯曲的最小角度差
// 范围 20°-80°，越小要求握拳越紧
const val BENT_THRESHOLD = 45f

// 是否要求其他手指弯曲（握拳检测开关）
const val REQUIRE_OTHER_FINGERS_BENT = true
...
4. **调整灵敏度（如需要）**
   - 在界面下方找到"手势灵敏度设置"卡片
   - **伸直严格度**：控制食指需要多直才能触发（20°-60°）
   - **握拳严格度**：控制其他手指需要多弯才能触发（20°-80°）
   - **握拳要求开关**：关闭后可仅用单根食指触发（不检测握拳）
...
### v2.4 (2026-03-22) - 添加握拳检测和灵敏度调节

**功能改进**:
- ✨ 新增：握拳检测功能，要求中指、无名指、小指必须弯曲才能触发手势（食指伸直+其他手指弯曲）
- ✨ 新增：界面灵敏度调节功能，用户可通过 SeekBar 实时调整伸直严格度（20°-60°）和握拳严格度（20°-80°）
- ✨ 新增：握拳要求开关，可切换"必须握拳"和"单手食指"两种模式

### v2.3 (2026-03-22) - 添加食指伸直检测

**功能改进**:
- ✨ 新增：食指伸直检测功能，握拳时不再误触发。只有当食指三段关节（MCP-PIP-DIP-TIP）方向一致时（角度差<40°）才识别为有效手势
...
**问题1**：握拳时食指弯曲但仍可能触发滑动（太灵敏）

**解决方案**：
1. 在界面上调整"伸直严格度"滑块（推荐）
   - 减小到 20°-30°：需要食指更直才能触发
2. 或在代码中修改：
```kotlin
val STRAIGHT_THRESHOLD = 25f  // 默认 40f，越小越严格
```
...
**解决方案**：
1. 在界面上调整"伸直严格度"滑块（推荐）
   - 增大到 50°-60°：允许食指有一定弯曲也能触发
2. 检查"握拳严格度"是否设置过高
   - 减小到 30°-40°：允许其他手指稍微伸直也能触发

**问题3**：不想握拳，只想用单根食指触发

**解决方案**：
关闭界面上的"必须握拳"开关，切换到单手食指模式。

**参数调节建议**：
| 问题场景 | 伸直严格度 | 握拳严格度 | 握拳要求 |
|----------|-----------|-----------|----------|
| 正常吃饭场景 | 40° | 45° | 开启 |
| 误触发频繁 | 25°-30° | 35°-40° | 开启 |
| 难以触发 | 50°-55° | 30°-35° | 开启 |
| 快速操作 | 45° | 40° | 关闭 |
